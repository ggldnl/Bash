...
# [all]
# Enumerate all the repositories with pending changes to be committed and pushed
# Usage: git_pushable | git_pushable <directory>
function git_pushable () {
	...
	if [ $# -eq 0 ]; then
		...
		echo "Usage: git_pushable | git_pushable <directory>"
		return
	fi

	for f in $dir/*; do 
		if [ -d $f/.git ]; then
			if [ $(git -C $f status --porcelain | wc -l) -ne "0" ]; then
			# git status --porcelain --untracked-files=no -> to avoid untracked files
				echo $f
			fi
		fi
	done;
}
export -f git_pushable

# [all]
# Enumerate all the repositories for which changes exists in the remote
# Usage: git_pullable | git_pullable <directory>
function git_pullable () {
	...
	if [ $# -eq 0 ]; then
		...
		echo "Usage: git_pullable | git_pullable <directory>"
		return
	fi

	for f in $dir/*; do 
		if [ -d $f/.git ]; then
			if [ $(git -C $f fetch --dry-run | wc -l) -ne "0" ]; then
				echo $f
			fi
		fi
	done;
...
function list_audio_devices() {
	sound_servers=$(inxi -Ax | grep -E 'PulseAudio|PipeWire')
	echo $sound_servers | grep -q '.*PipeWire.*running: yes' && echo "PipeWire:" && pw-cli list-objects | grep node.name
	echo $sound_servers | grep -q '.*PulseAudio.*running: yes' && echo "PulseAudio:" && pacmd list-sources | grep -e 'index:' -e device.string -e 'name:'
}
